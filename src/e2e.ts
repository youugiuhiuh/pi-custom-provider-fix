...
const REAL_FILE = path.join(os.homedir(), ".pi", "agent", "models-custom.json");
const BACKUP = fs.existsSync(REAL_FILE) ? fs.readFileSync(REAL_FILE) : null;

let ok = 0, fail = 0;
function t(name: string, fn: () => boolean) {
  try { if (fn()) { ok++; } else { console.log("FAIL:", name); fail++; } }
  catch (e: any) { console.log("CRASH:", name, e.message); fail++; }
}

const ENTER = "\r", ESC = "\x1b", UP = "\x1b[A", DOWN = "\x1b[B", LEFT = "\x1b[D";
const M = (id: string, sel = false) => ({ id, name: id, reasoning: false, input: ["text"], contextWindow: 128000, maxTokens: 16384, selected: sel, edited: false });

// ─── Setup ───────────────────────────────────────────────────────
fs.writeFileSync(REAL_FILE, JSON.stringify({ providers: {} }));

// ─── 1. CREATE provider with apiKey ──────────────────────────────
t("CREATE: wizard saves apiKey", () => {
  const s = createWizardState([]);
  s.step = "api_type"; handleWizardInput(s, ENTER);
  s.baseUrl = "https://test.com/v1"; handleWizardInput(s, ENTER);
  ...
  s.discoveryLoading = false; s.step = "review";
  handleWizardInput(s, ENTER); // save action

  // Execute save
  writeConfig(addProvider(readConfig(), s.providerId, {
    baseUrl: s.baseUrl, api: s.apiType,
    apiKey: s.apiKey || readConfig().providers[s.providerId]?.apiKey || undefined,
    authHeader: true, models: s.discoveredModels.filter(m => m.selected).map(m => ({
      id: m.id, input: m.input, contextWindow: m.contextWindow, maxTokens: m.maxTokens
    }))
  }));
...
t("BUG FIX: apiKey preserved when s.apiKey empty", () => {
  ...
  const emptyApiKey = "";
  writeConfig(addProvider(cfg, "test-prov", {
    ...p, baseUrl: p.baseUrl, api: p.api,
...
t("EDIT CONFIG: apiKey survives config save", () => {
  ...
  writeConfig(addProvider(cfg, "test-prov", {
    ...p, baseUrl: "https://changed.com", api: p.api,
...
t("SAVE MODELS: {...p} preserves apiKey", () => {
  ...
  const p = cfg.providers["test-prov"];
  writeConfig(addProvider(cfg, "test-prov", {
    ...p, models: [M("new-model")].map(m => ({ id: m.id, input: m.input, contextWindow: m.contextWindow, maxTokens: m.maxTokens }))
  }));
  return readConfig().providers["test-prov"].apiKey === "sk-test-key-123";
});
...
t("ESC: navigation chain", () => {
  const cases: [string, string][] = [
    ["api_type", "choose_provider"], ["base_url", "api_type"],
    ["api_key", "base_url"], ["provider_id", "api_key"],
    ["edit_model", "select_models"], ["review", "select_models"],
    ["manage_config", "select_models"],
  ];
  return cases.every(([from, to]) => {
    const s = createWizardState([]);
    s.step = from as any; s.providerId = "x";
...
// ─── Cleanup ─────────────────────────────────────────────────────
if (BACKUP) fs.writeFileSync(REAL_FILE, BACKUP);
else fs.unlinkSync(REAL_FILE);
